    ...
    link:function (originalScope, element, attrs, modelCtrl) {
      ...
      var getMatchesAsync = function(inputValue) {
        ...
        if(inputValue != undefined){
          matcher = inputValue.match(/([^,\s]+)$/)
          if(matcher){
            inputValue = matcher[0]
          }
        }
        ...
        // console.log(parserResult.source(scope, locals))
        $q.when(parserResult.source(scope, locals)).then(function(matches) {
          ...
          //
          if (new RegExp(inputValue).test(totalInput)) {
            if(!matches){
              matches = [];
            }
            if (matches.length > 0) {
      ...
      modelCtrl.$render = function () {
        ...
        var suggestion = modelCtrl.$viewValue || ''
        var result = totalInput.replace(new RegExp(matcher + '\s*$'), suggestion)
        // element.val(parserResult.viewMapper(scope, locals) || modelCtrl.$viewValue);
        $viewValue = result;

        element.val(result || parserResult.viewMapper(scope, locals));
        selected = undefined;
      };
      ...
      //element is the input component
...
.directive('typeaheadPopup', function () {
  return {
    ...
    replace:true,
    templateUrl:'/assets/typeahead/typeahead.html',
